...
Fortress *is* the browser for your agent — raw CDP on `:9222`, no stealth plugins to wire up. **Open it pre-loaded in a chat assistant** (it reads our [AGENTS.md](AGENTS.md) and sets everything up), or **copy the prompt block** below (hit the copy button, top-right) into Claude Code, Cursor, or any coding agent:
